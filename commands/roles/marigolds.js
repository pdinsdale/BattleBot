module.exports = {
    ...
    let faction1 = client.roles(`${client.guildConfig.faction1Role}`);
    let faction2 = client.roles(`${client.guildConfig.faction2Role}`);

    // If user has the other faction role, remove it
    if (message.member.roles.has(faction2.id)) {
        message.member.removeRole(faction2).catch((err) => console.log(err));
    }
