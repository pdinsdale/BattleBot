module.exports = { name: 'command-name', aliases: ['mariogolds', 'marigold', 'mariogold'], async run(client, message, args) {
    ...
    if (message.member.roles.has(faction1.id)) {
        ...
        message.member.addRole(faction1).catch(err => message.reply(`I couldn't apply the role because <@${client.config.ownerID} screwed something up in my code. Please ping an online mod to manually apply the role for you. \n\nThis was the error: ${err}`));
