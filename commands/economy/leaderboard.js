module.exports = { name: 'leaderboard', description: 'Shows the top 10 users on the server. If a user is mentioned, tells the position of the user on the leaderboard', aliases: ['lb'], usage: '[@User]', args: '[@User] => (Optional) Any valid member of the server', modonly: false, async run(client, message, args, Discord, eco) {
    ...
    if (message.mentions.users.first()) {
        ...
        eco.Leaderboard({
          ...
          if (users[8]) var ninethplace = await client.fetchUser(users[8].userid);
  ...
  **1 - ${firstplace && firstplace.tag || 'Nobody Yet'} : Balance - ${users[0] && users[0].balance || 'None'}
  ...
  4 - ${thirdplace && fourthplace.tag || 'Nobody Yet'} : Balance - ${users[3] && users[3].balance || 'None'}
  5 - ${thirdplace && fifthplace.tag || 'Nobody Yet'} : Balance - ${users[4] && users[4].balance || 'None'}
  6 - ${thirdplace && sixthplace.tag || 'Nobody Yet'} : Balance - ${users[5] && users[5].balance || 'None'}
  7 - ${thirdplace && seventhplace.tag || 'Nobody Yet'} : Balance - ${users[6] && users[6].balance || 'None'}
  8 - ${thirdplace && eighthplace.tag || 'Nobody Yet'} : Balance - ${users[7] && users[7].balance || 'None'}
  9 - ${thirdplace && ninethplace.tag || 'Nobody Yet'} : Balance - ${users[8] && users[8].balance || 'None'}
  10 - ${thirdplace && tenthplace.tag || 'Nobody Yet'} : Balance - ${users[9] && users[9].balance || 'None'}**`);
